...
.bploop_impl <-
    function(ITER, FUN, ARGS, BPPARAM, BPREDO,
             OPTIONS, reducer, progress.length)
{
...
bploop.lapply <-
    function(manager, X, FUN, ARGS, BPPARAM, BPREDO = list(), OPTIONS, ...)
{
...
bploop.iterate <-
    function(
        manager, ITER, FUN, ARGS, BPPARAM, REDUCE, BPREDO, OPTIONS,
